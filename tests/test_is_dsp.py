...
def test_is_dsp():
    ...
    assert not saddle_problem.is_dsp() # AssertionError: Cannot add variables to both convex and concave set.

    obj = MinimizeMaximize(weighted_log_sum_exp(x,y) + z)
    saddle_problem = SaddlePointProblem(obj, [cp.sum(x) == 1, cp.sum(y) == 1])

    assert not saddle_problem.is_dsp() # AssertionError: Cannot resolve curvature of variables ['z']. Specify curvature of these variables as SaddleProblem(obj, constraints, minimization_vars, maximization_vars).
    ...
    with pytest.raises(AssertionError):
        saddle_problem = SaddlePointProblem(obj, [cp.sum(x) == 1, cp.sum(y) == 1], minimization_vars=[x], maximization_vars=[y])
...
def test_problem_is_dcp():
    ...
    sup_y_f = saddle_max(f, [y], [cp.sum(y) == 1])
...
def test_saddle_extremum_local_affine():
    ...
    F = saddle_max(f, [y], [cp.sum(y) == 1])
    assert not F.is_dsp() # z is a local variable appearing in the objective, and not listed as a concave variable
    ...
    assert not G.is_dsp() # z is a local variable appearing in the objective, and not listed as a convex variable
    

def test_saddle_extremum_missing_local():
    x = cp.Variable(2, name="x", nonneg=True)
    y = LocalVariable(2, name="y_local1", nonneg=True)
    z = cp.Variable(name="z")
    f = weighted_log_sum_exp(x,y) + z
    ...
    f = weighted_log_sum_exp(x,y) + cp.log(z)
    ...
    f = weighted_log_sum_exp(x,y) + cp.exp(z)
    F = saddle_max(f, [y_local], [cp.sum(y_local) == 1])
    ...
    # why does the local variable already have an expression?
    print("testing")
    F = saddle_min(f, [x], [cp.sum(x) == 1])
    # assert f.is_dsp()
    # assert not F.is_dsp()
